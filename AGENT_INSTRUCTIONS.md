# Agent Instructions: Atlanta Sharded Extraction

We are running a distributed extraction for "Greater Atlanta" zip codes (300-303) across 4 separate instances to avoid rate limiting.
...
```bash
python scripts/extract_pricing_atl.py --zip-file data/shards_atlanta/shard_atl_X.json --items "Mattress,Bag of Junk:5" --workers 5
```
*(Replace `X` with your assigned shard number)*
